...
4. 本应用仅限学习和内部使用，严禁商用和用作其他非法用途，如有违反，与本人无关！！！  
   **如果要用，请做好隐蔽工作，`不要被人发现`！如果被发现（还是别被发现了），后果自负。**</br>
   **如果要用，请做好隐蔽工作，`不要被人发现`！如果被发现（还是别被发现了），后果自负。**</br>
   **如果要用，请做好隐蔽工作，`不要被人发现`！如果被发现（还是别被发现了），后果自负。**</br>

# 最新版本 2.1.1 —— 更新时间：2025年6月16日09点29分
...
1. 添加每日任务重置时间点设置
2. 添加下拉刷新任务列表功能，解决任务删除小概率会失败的问题
3. 重构消息事件处理机制
4. 优化任务列表刷新功能

#### [DT_20250616_2.1.1.0.apk，适合Android 8+系统](apk/daily/release/DT_20250616_2.1.1.0.apk)

1. 修改前台服务通知标题
2. 优化从钉钉返回软件主页面的逻辑
3. 优化保活服务和后台计时服务
4. 优化任务状态更新逻辑
5. 日常更新，规范版本命名规则，动态编译更新包名，防止长时间固定使用同一个包名被检测到
